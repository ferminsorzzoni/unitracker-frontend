...
export default function DeleteSubcategoryButton({ subcategoryId }: { subcategoryId: string }) {
    const dialogRef = useRef<HTMLDialogElement>(null);
    const { subcategoryActions } = useCareerContext();

    const onDelete = () => {
        subcategoryActions.delete(subcategoryId, {
            onSuccess: () => {
    ...
    return (
        <>
            ...
            >
                <img src={thrashIcon} alt="Borrar subcategoría" className="w-4 h-4"/>
            </button>
            ...
            >
                <h2>¿Estás seguro de borrar esta subcategoría?</h2>
